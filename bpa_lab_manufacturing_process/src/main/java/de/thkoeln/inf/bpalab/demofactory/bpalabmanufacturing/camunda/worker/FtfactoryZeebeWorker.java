...
import de.thkoeln.inf.bpalab.demofactory.bpalabmanufacturing.camunda.message.FtfactoryHBWStartMessage;
...
public class FtfactoryZeebeWorker extends AWorker {
	...
	public Map<String, Object> ensureStorageAvailable(final ActivatedJob job) {
		...
		String	orderType = (String) variables.get("orderType");
		String manufactureOrderCorrelation = job.getVariable("manufactureOrderCorrelation").toString();

		ftfactoryHBWStartMessage.setReplyMessageCorrelationValue(manufactureOrderCorrelation);
			
		HashMap<String, Object> sendVariables = new HashMap<>();
		sendVariables.put("orderType", orderType);
		sendVariables.put("manufactureOrderCorrelation", manufactureOrderCorrelation);
